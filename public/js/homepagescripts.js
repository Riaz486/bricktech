...
function naylaDynamicCarousel(){const carousel=$('.nayla-dynamic-carousel');carousel.each(function(){let carousel=$(this),wrapper=carousel.children('.carousel--wrapper'),items=wrapper.children('.carousel--item'),end,length=items.length,parentLeft,next=carousel.find('.carousel--next'),prev=carousel.find('.carousel--prev'),animation=carousel.data('animation'),customPin=carousel.data('pin'),speed=carousel.data('scroll-speed'),carouselDrag,wrapScroll,val;val=0;end=wrapper.outerWidth()-items.last().outerWidth()-(items.last().outerWidth()/2);parentLeft=carousel.offset().left,carousel.find('.carousel--total').html(length);items.each(function(i){let $this=$(this);$this.attr('data-index',i);$this.addClass('carousel--item--'+i);if(carousel.hasClass('parallax-on')){$this.find('img').wrap('<div class="parallax-wrap"></div>');$this.find('.parallax-wrap').css('height',$this.find('img').outerHeight());$this.find('.parallax-wrap').css('width',$this.find('img').outerWidth());$this.find('img').css('width',mobileQuery.matches?'calc(100% + 50px)':'calc(100% + 100px)');$(window).on('resize',()=>{$this.find('.parallax-wrap').css('width',$this.outerWidth());$this.find('img').css('width',mobileQuery.matches?'calc(100% + 50px)':'calc(100% + 100px)');})}
...
function naylaMarquee(i){var velocity;let marquee=$('.nayla-marquee');marquee.each(function(){let $this=$(this),text=$this.children('*'),dataDuration=$this.data('duration'),seperator=$this.data('seperator');text.append('<span class="material-icons">'+seperator+'</span>')
...
function naylaAccordion(){let naylaAccordion=$('.nayla-accordion');naylaAccordion.each(function(){let $this=$(this),acrdTitle=$this.find('.accordion-title'),acrdContent=$this.find('.accordion-content'),activeContent=$this.find('.active > .accordion-content'),icon=$this.data('toggle-icon'),animation=$this.data('animation');if(animation){acrdTitle.wrapInner('<span></span>')
...
function naylaClinetsCarousel(){let clientCarousel=$('.nayla-clients-carousel');clientCarousel.each(function(){let $this=$(this),clientWrap=$this.find('.clients-wrapper'),clientFullWidth=clientWrap.outerWidth(),dataDuration=$this.data('duration'),windowWidth=window.innerWidth,tl=gsap.timeline();if(clientFullWidth<windowWidth){let i=0,clientLenght=Math.ceil(windowWidth*2/clientFullWidth);for(i=1;i<=clientLenght;i++){clientWrap.clone().appendTo($this)}
...
function initPageElements(){naylaIcons()
naylaHeading()
naylaCartButton()
naylaTeamMember()
...
function initShowcases(){showcaseWall();showcaseList();showcaseFullscreenSlideshow();showcaseMinimalList();showcaseCarousel();showcaseInfiniteGrid();showcaseFullscreenCarousel();horizontalList();showcaseFullscreenWall();showcaseInteractiveGrid()
...
naylaMouseCursor(true);smoothScroll();$(window).on('load',function(){winLoaded=true;window.scrollTo(0,0);naylaVideo($('.nayla-video'));naylaGeneralAnims($('.has-anim'));naylaTextAnims();naylaTextWrapper();naylaListAnimations();naylaImageAnims()
